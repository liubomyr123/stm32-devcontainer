...
public:
    ...
    void log(LogLevel level, const char* tag, const char* file, int line, const char* fmt, va_list args);
...
#define LOG_DEBUG(tag, fmt, ...)   Logger::instance().debug(tag, __FILE__, __LINE__, fmt, ##__VA_ARGS__)
#define LOG_INFO(tag, fmt, ...)    Logger::instance().info(tag, __FILE__, __LINE__, fmt, ##__VA_ARGS__)
#define LOG_WARNING(tag, fmt, ...) Logger::instance().warning(tag, __FILE__, __LINE__, fmt, ##__VA_ARGS__)
#define LOG_ERROR(tag, fmt, ...)   Logger::instance().error(tag, __FILE__, __LINE__, fmt, ##__VA_ARGS__)
