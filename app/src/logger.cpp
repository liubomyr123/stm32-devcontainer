...
void Logger::log(LogLevel level, const char* tag, const char* file, int line, const char* fmt,
                 va_list args)
{
    if (level < minLevel_)
    {
    ...
    printf("[%02u:%02u:%02u.%03u][%s][%s][%s:%d]: ", (unsigned int)hour, (unsigned int)min,
